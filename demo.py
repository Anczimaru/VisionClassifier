...
################################################################################
@timing
def main(debug_mode=1, full_run = 1):
    for f in config.directories_demo:
        if not os.path.exists(f):
            os.mkdir(f)
            s = "Making directory {}".format(f)
        if not os.path.exists(config.data_CHIC_demo_dir):
            os.mkdir(config.data_CHIC_demo_dir)
            print("Please place CHIC Dataset in proper directory")
    if (full_run == 1):
        #IF NO FILES DO EDGE DETECTION
        prepare_data(picture.canny_edge_detector, config.data_CHIC_demo_dir, config.edges_demo_dir, config.s_base_edge_name, config.data_demo_dir)

        prepare_data(picture.laplacian, config.data_CHIC_demo_dir, config.laplacian_demo_dir, config.s_base_laplace_name, config.data_demo_dir)

        prepare_data(picture.sobel_x, config.data_CHIC_demo_dir, config.sobel_x_demo_dir, config.s_base_sobel_x_name, config.data_demo_dir)

        prepare_data(picture.sobel_y, config.data_CHIC_demo_dir, config.sobel_y_demo_dir, config.s_base_sobel_y_name, config.data_demo_dir)

    with open(os.path.join(config.root_demo_dir,"log_edge.txt"),"w") as log_file:
        print("Evaluating edges")
        evaluate_intersection(config.edges_demo_dir, config.s_base_edge_name, log_file, config.data_demo_dir)

    with open(os.path.join(config.root_demo_dir,"log_laplacian.txt"),"w") as log_file:
        print("Evaluating laplacian")
        evaluate_intersection(config.laplacian_demo_dir, config.s_base_laplace_name, log_file, config.data_demo_dir)

    with open(os.path.join(config.root_demo_dir,"log_sobel_x.txt"),"w") as log_file:
        print("Evaluating sobel_x")
        evaluate_intersection(config.sobel_x_demo_dir, config.s_base_sobel_x_name, log_file, config.data_demo_dir)


    with open(os.path.join(config.root_demo_dir,"log_sobel_y.txt"),"w") as log_file:
        print("Evaluating sobel_y")
        evaluate_intersection(config.sobel_y_demo_dir, config.s_base_sobel_y_name, log_file, config.data_demo_dir)


        log_file.close()


    merge_results(config.root_demo_dir, config.data_CHIC_demo_dir)
...
def show_result_on_picture(index):
    for f in os.listdir(config.data_CHIC_demo_dir):
            ...
            with open(os.path.join(root_dir,"merged_logs.txt"), "r") as log_file:
                ...
                for line in lines:
                    if find_level_merged(line) == level:
                        ...
                        img = cv2.imread(os.path.join(config.data_CHIC_demo_dir,f))
